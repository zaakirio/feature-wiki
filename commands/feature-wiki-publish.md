...
1. For each built doc in `.featurewiki/docs/`, compute its `fwId` and marker from the feature record
   and `scripts/manifest.py`.
2. SEARCH the live target for an existing page bearing that marker (content property / label / line-1
   marker comment), and also check the local manifest.
...
5. Record each published page with `scripts/manifest.py record --fw-id <fwId>`.
